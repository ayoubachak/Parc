...
import {vehicleMapping} from "./data";
...
const VehicleEdit = ()=>{
    ...
    // now let's actually get the vehicle and place the values where they belong
    ...
    useEffect(() => {
        const fetchData = async () =>{
            ...
            if(data?.id){
                setVehicle(data)
                setColor(data.color)
                setLiscence(data.liscence)
                setModel(data.model)
                setNumChairs(data.numchairs)
                setPower(data.power)
                setVehkm(data.vehkm)
                setBrand(data.brandModel.brand)
    ...
    // this should track the brand change
    useEffect(()=>{
        if(brands.length > 0){
            setBrandModels(brandModelsByBrandId(brand.id))
        }
        if(brandModels.length > 0){
            setBrandModel(vehicle.brandModel)
        }
    },[brands, brandModels, vehicle])
    ...
    useEffect(() => {
        const addVehicle = async ()=>{
            if (
                ...
                console.log("Saveing the vehicle")
                const response = await vehicleService.edit(id, vehicle);
                if(response.status === 200){
                    handleSuccessNotification("Vehicle Saveed Successfully")
                    setTimeout(()=>{
                                            ...
                                            <Grid item xs={12}>
                                                <FormControl variant="outlined">
                                                    ...
                                                    <Select
                                                        ...
                                                        value={brandModel && brandModel.id ? brandModel.id : ""}
                                                        style={{ minWidth: "150px" }}
                                            ...
                                            <Grid item xs={12}>
                                                <FormControl variant="outlined">
                                                    ...
                                                    <Select
                                                        ...
                                                        value={fuelType && fuelType.id ? fuelType.id : ""}
                                                        style={{ minWidth: "150px" }}
